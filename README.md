...
```
dbx execute --cluster-name=<name of interactive cluster> --job=cicd_demo_temp-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx launch --job=cicd_demo_temp-sample-integration-test
```
...
```
dbx launch --job=cicd_demo_temp-sample
```
